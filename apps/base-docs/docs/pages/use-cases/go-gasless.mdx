...
You will create a constant for our Paymaster & Bundler endpoint obtained from portal.cdp.coinbase.com. The most secure way to do this is by using a proxy. For the purposes of this demo, hardcode it into our `index.js` file. For product, we highly recommend using a [proxy service].
:::
