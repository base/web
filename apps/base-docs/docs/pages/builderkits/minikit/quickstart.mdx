...
MiniKit is easiest way to build mini apps on Base, allowing developers to easily build applications without needing to know the details of the SDK implementation. It integrates seamlessly with [OnchainKit](https://www.base.org/builders/onchainkit) components and provides Coinbase Wallet-specific hooks that adapt smoothly to work with other clients such as Warpcast.

## Prerequisites

Before you begin developing with MiniKit, you'll need:

1. **Warpcast Account**: Create an account on [Warpcast](https://warpcast.com/) to test and deploy your Frames.
...
MiniKit streamlines app development by providing a comprehensive toolkit that makes the Frames SDK more intuitive:

- **Simplified Development:** Build apps with minimal knowledge of the Frames SDK
- **Coinbase Wallet Integration:** Access Coinbase Wallet-specific hooks that adapt smoothly to other clients
...
:::info
We recommend using [Vercel](https://vercel.com) to deploy your MiniKit app, as it integrates seamlessly with the upstash/redis backend required for stateful frames, webhooks, and notifications. The CLI will guide you through setting up the necessary environment variables for your Redis database.

We recommend Vercel to test your mini app, however, for local development, you can use [ngrok](https://ngrok.com/). Note that ngrok URLs change each session unless using a paid account, which means you'll need to recreate the manifest each time you restart ngrok.

To deploy the example app to Vercel:
...
## Testing Your mini app in [Warpcast](https://warpcast.com/)
...
The `MiniKitProvider` wraps your application and provides global access to the SDK's context. It handles initialization, events, and automatically applies client [safeAreaInsets](https://docs.farcaster.xyz/developers/frames/v2/spec) to ensure your app doesn't overlap parent application elements.
...
2. **Account Association Generation**
  ...
  To prove domain ownership, a Farcaster account signs a JSON payload with its custody address. This verification process is called [account association](https://docs.farcaster.xyz/developers/frames/v2/spec) by Farcaster.
