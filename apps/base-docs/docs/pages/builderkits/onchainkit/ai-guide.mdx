...
7. Constraints and Considerations: Performance, security, and accessibility requirements
8. Getting Started: Step-by-step setup instructions for new developers

Here's my current project information:
[paste relevant project details here]
```
...
After creating your project, use the same instructions.md prompt to generate comprehensive documentation for your new OnchainKit project:

```
I've just created a new project with 'npm create onchain@latest'. Based on the initial project structure, please create a detailed instructions.md file following the format we discussed, focusing specifically on:

1. The OnchainKit components and their purposes
2. How smart contract integrations should be organized
3. Recommended folder structure for blockchain-specific functionality
4. Key considerations for Web3 authentication and interactions
...
```
I'm working on a [project type] using [frameworks/libraries]. I need your help with:
...
```
I'm implementing a wallet connection component with OnchainKit. Here's the relevant LLMs.txt:
...
1. Supports multiple wallet providers
...
```
I need to implement a new feature in my project. Here's the relevant section from my instructions.md file:
...
Based on this context, please help me implement a token balance display component that:
1. Shows the connected wallet's balance of our ERC20 token
2. Updates when the balance changes
3. Handles loading and error states appropriately
4. Follows our project's coding standards
```

## **\*if you are using cursor, you can simply add the instructions.md into context like below.**
...
2.   
   **Format specification**: Request specific formats for clarity
...
3.   
   **Length guidance**: Indicate whether you want brief or detailed responses
...
4.   
   **Clarify ambiguities**: Help resolve unclear points when you receive multiple options
