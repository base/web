...
The Profiles feature allows developers to request personal information from Smart Wallet users during a
...
1. **Success Response** - Return the original calls to proceed with the transaction:
...
2. **Return Original Calls**: You MUST return the original calls in your success response. If you don't, the wallet will return an error.
...
- Coinbase Wallet SDK v4.4.0 or higher
- An app connected to an EVM-compatible chain
- A secure HTTPS endpoint for data validation
