...
Install viem for onchain interactions
...
- `enableAutomaticSubAccountCreation`: When set to true, automatically creates a Sub Account at connection
- `spendPermissionConfig`: Configures Spend Permissions for Sub Account, including:
  - Token address (In this case, `0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE` represents the native ETH)
  - Allowance amount (in hex)
