...
// SubAccount Illustration Component
const SubAccountIllustration = ({
  universalAccount,
  subAccount,
  ...
  universalAccountSigning: () => Promise<void>;
  subAccountSigning: () => Promise<void>;
}) => {
  return (
      ...
      <div className="mt-8 max-w-lg rounded-xl border border-slate-700/30 bg-gradient-to-r from-slate-800/50 to-slate-900/50 p-6 text-center text-sm text-slate-300 shadow-sm">
        ...
        <ul className="mt-4 list-none space-y-2 text-left">
          <li className="flex items-center">
            <span className="mr-2 inline-block h-4 w-4 rounded-full bg-sky-500 opacity-80"></span>
            <span>Manage its own assets and sign transactions independently</span>
          </li>
          <li className="flex items-center">
            <span className="mr-2 inline-block h-4 w-4 rounded-full bg-teal-400 opacity-80"></span>
            <span>Spend assets from universal account using user-approved Spend Permission</span>
          </li>
          <li className="flex items-center">
            <span className="mr-2 inline-block h-4 w-4 rounded-full bg-blue-400 opacity-80"></span>
...
export default function SubAccount() {
  ...
  useEffect(() => {
    ...
    getCryptoKeyAccount().then((signer) => {
      setSigner(signer);
    });
  }, []);
  ...
  return (
    <>
      {subAccount ? (
        <>
          {/* <Address address={subAccount} className="text-lg" /> */}
        ...
        <Button className="border-none bg-sky-600" onClick={createSubAccount}>
          Create Sub Account
        </Button>
      )}
    </>
